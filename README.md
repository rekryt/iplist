...
в конфиге портала можно задать «точечные» замены:
...
Семантика работает в две фазы.

**Reload-time (при обновлении данных портала).** В конце каждого цикла
`reload`, если выставлен ENV `SYS_REPLACE_ESCALATE_IPS=true` (по умолчанию),
сервис проходит по ключам `replace.cidr4`/`replace.cidr6` и добавляет
в массивы-значения все `ip4`/`ip6` портала, попадающие в ключевую зону,
с маской `/32` (или `/128` для IPv6). К каждому массиву-значению
применяется `minimizeSubnets` — поглощённые админом более узкой зоны
записи исчезают, повторный `reload` не плодит дубликатов. Результат
сохраняется в JSON конфига.

**View-time (при ответе HTTP-клиенту).** Все форматы, кроме `json`,
выкидывают из `cidr4`/`cidr6` ключевые зоны `replace` и подставляют
вместо них значения. После агрегации между сайтами применяется
`minimizeSubnets`, дубликатов в ответе нет.

`json` намеренно возвращает сырое состояние портала вместе с полем
`replace` — клиент может сам решать, как его интерпретировать.

Замена НЕ применяется к `ip4`/`ip6`/`domains` и не мутирует `cidr4`/`cidr6`
в памяти или в JSON — только растит value-массивы внутри самого `replace`.
Ключи сравниваются строкой байт-в-байт: пишите их ровно в той форме, в
какой они появляются в `cidr4`/`cidr6` портала. Некорректная структура
`replace` (значение не object / не array / не строка) приводит к ошибке
загрузки и остановке сервера.
